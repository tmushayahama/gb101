  ...
  public static function getChildrenTodos($todoParentId) {
	...
	public function rules()
	{
		...
		return array(
			...
			array('parent_todo_id, priority_id, creator_id, assignee_id, type', 'numerical', 'integerOnly'=>true),
			array('todo_color', 'length', 'max'=>6),
			array('description', 'length', 'max'=>500),
			array('due_date', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, parent_todo_id, priority_id, creator_id, assignee_id, created_date, due_date, todo_color, description, type', 'safe', 'on'=>'search'),
		);
	}
