...
class TimelineController extends Controller {
 ...
 public function accessRules() {
  return array(
    array('allow', // allow all users to perform 'index' and 'view' actions
      'actions' => array('index', 'timelinebank', 'timelinebankdetail', 'appendMoreTimeline'),
      'users' => array('*'),
    ),
    array('allow', // allow authenticated user to perform 'create' and 'update' actions
      'actions' => array('addTimelineReply'),
      'users' => array('@'),
    ),
 ...
 public function actionAddTimelineReply() {
 ...
 public function actionAppendMoreTimeline() {
  if (Yii::app()->request->isAjaxRequest) {
   $nextPage = Yii::app()->request->getParam('next_page') * 100;
   $type = Yii::app()->request->getParam('type');
   $bankSearchCriteria = Bank::getBankSearchCriteria(TimelineType::$CATEGORY_SKILL, null, 100, $nextPage);
   switch ($type) {
    case 1:
     echo CJSON::encode(array(
       '_timeline_bank_list' => $this->renderPartial('timeline.views.timeline._timeline_bank_list', array(
         'timelineBank' => Bank::model()->findAll($bankSearchCriteria))
         , true
     )));
     break;
    case 2:
     echo CJSON::encode(array(
       '_timeline_bank_list' => $this->renderPartial('timeline.views.timeline._timeline_bank_list_1', array(
         'timelineBank' => Bank::model()->findAll($bankSearchCriteria))
         , true
     )));
     break;
