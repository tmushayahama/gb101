...
class PromiseController extends Controller {
 ...
 public function accessRules() {
  return array(
    ...
    array("allow", // allow authenticated user to perform "create" and "update" actions
      "actions" => array("promiseHome", "promisebank", "addpromise", "addPromiseComment", "addPromiseContributor",
        "addPromiseQuestionnaire", "addPromiseTodo", "addPromiseDiscussion", "AddPromiseWeblink",
        "addPromiseNote", "addPromiseTimeline"),
      "users" => array("@"),
    ),
 ...
 public function actionAddPromiseTimeline($promiseId) {
  if (Yii::app()->request->isAjaxRequest) {
   if (isset($_POST["Timeline"]) && isset($_POST["PromiseTimeline"])) {
    $timelineModel = new Timeline();
    $promiseTimelineModel = new PromiseTimeline();
    $timelineModel->attributes = $_POST["Timeline"];
    $promiseTimelineModel->attributes = $_POST["PromiseTimeline"];
    if ($promiseTimelineModel->validate() && $timelineModel->validate()) {
     ...
     if ($timelineModel->save(false)) {
      ...
      echo CJSON::encode(array(
        ...
        "source_pk_id" => 0,
        "_post_row" => $this->renderPartial("promise.views.promise.activity._promise_timeline_item_row", array(
          "promiseTimeline" => PromiseTimeline::getPromiseTimeline($promiseId),
          )
          , true)
      ));
     ...
     echo CActiveForm::validate(array($promiseTimelineModel, $timelineModel));
    }
   }
   Yii::app()->end();
  }
 ...
 public function actionAddPromiseContributor($promiseId) {
  if (Yii::app()->request->isAjaxRequest) {
