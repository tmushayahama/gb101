...
class Hobby extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('skill_id, status', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>100),
			array('description', 'length', 'max'=>500),
			array('created_date', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, skill_id, title, description, created_date, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'hobbyLists' => array(self::HAS_MANY, 'HobbyList', 'hobby_id'),
		);
	}
